...
# 💡 [신규 무기] 알프레도의 수급(투자자별 매매동향) 탐지기 추가
def tool_get_investor_trend(name):
    try:
        ...
        if len(tables) < 2: return json.dumps({"error": "데이터 테이블을 찾을 수 없습니다."})
        ...
        for r in rows:
            ...
            if len(cols) >= 9:
                ...
                if date_str.count('.') == 2:
                    results.append({
                        "날짜": date_str,
                        "종가": cols[1].text.strip(),
                        "기관순매매량": cols[5].text.strip(),
                        "외국인순매매량": cols[6].text.strip()
                    })
            if len(results) >= 5:
                break
                
        return json.dumps({"종목명": name, "최근_5거래일_수급": results}, ensure_ascii=False)
    except Exception as e: 
        return json.dumps({"error": f"수급 조회 실패: {str(e)}"})

# 💡 [스키마 업데이트] get_investor_trend 추가
tools_schema = [
    ...
    {"type": "function", "function": {"name": "get_investor_trend", "description": "특정 종목의 최근 5거래일 기관 및 외국인 수급 동향(순매매량)을 가져옵니다.", "parameters": {"type": "object", "properties": {"name": {"type": "string", "description": "종목명"}}, "required": ["name"]}}}
]
